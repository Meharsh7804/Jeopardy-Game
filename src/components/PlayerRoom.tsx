    ...
    <div className="min-h-screen flex flex-col select-none">
      ...
      <header className="glass-panel sticky top-0 z-40 px-5 py-3 flex items-center justify-between rounded-b-2xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-sm text-text-main leading-none">
              {myName}
            </p>
            <p className="text-[10px] text-text-muted">
              Room:{" "}
              <span className="font-bold text-primary-accent tracking-widest">
                {room.id}
              </span>
            </p>
          </div>
        ...
        <AnimatePresence mode="wait">
            ...
            >
              ...
              <div>
                ...
                <p className="text-sm text-text-muted mt-2">
                  You're in. Players will appear here as the room fills up.
                </p>
              </div>
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-semibold text-text-main truncate">
                            {p.name}{" "}
                            {p.id === myId && (
                              <span className="text-[10px] text-primary-accent">
                                (you)
                              </span>
                            )}
                          </p>
            ...
            >
                    ...
                    {Array.from({
                      ...
                      quiz.categories.map((cat) => {
                        ...
                        return (
                          <div
                            key={q.id}
                            className={`glass-panel rounded-xl p-2 font-bold text-sm text-center flex items-center justify-center min-h-11 ${done ? "opacity-20" : "text-[#FACC15]"}`}
                          >
                            {done ? "" : `$${q.value}`}
                          </div>
                        );
              ...
              <div className="glass-panel p-4 rounded-2xl space-y-3">
                ...
                {players.map((p, i) => (
                  <div
                    key={p.id}
                    className={`flex items-center justify-between px-3 py-2.5 rounded-xl border ${p.id === myId ? "bg-primary-accent/10 border-primary-accent/30" : "bg-card-bg/30 border-white/5"}`}
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      {i === 0 && p.score > 0 ? (
                        <Crown className="w-3.5 h-3.5 text-[#FACC15] fill-[#FACC15]" />
                      ) : (
                        <span className="text-[10px] text-text-muted w-3.5 text-center">
                          #{i + 1}
                        </span>
                      )}
                      <PlayerAvatar
                      ...
                      <span className="font-semibold text-sm text-text-main truncate">
                        {p.name}{" "}
                        {p.id === myId && (
                          <span className="text-primary-accent text-[10px]">
                            (you)
                          </span>
                        )}
                      </span>
            ...
            >
              <div className="glass-panel p-6 rounded-2xl text-center space-y-3">
                <div className="flex items-center justify-center gap-2">
                  ...
                  <span className="font-display font-extrabold text-[#FACC15]">
              ...
              {!hasBuzzed ? (
                ...
                <div className="flex flex-col items-center gap-3">
                  ...
                  >
                    <Zap
                      className={`w-10 h-10 ${hasBuzzed ? "" : "fill-white/30"}`}
                    />
            ...
            >
              <div className="glass-panel p-6 rounded-2xl text-center space-y-3">
                ...
                <div className="p-4 rounded-xl bg-success-accent/10 border border-success-accent/30">
                  ...
                  <p className="text-xl font-display font-extrabold text-success-accent">
          ...
          {room.phase === "ended" && (
            ...
            >
              ...
              <h2 className="text-2xl font-display font-extrabold text-text-main">
                Game Over!
              </h2>
              <div className="space-y-2 w-full max-w-xs">
                {players.map((p, i) => (
                  <div
                    key={p.id}
                    className={`flex items-center justify-between px-4 py-3 rounded-xl border ${i === 0 ? "bg-[#FACC15]/10 border-[#FACC15]/30" : "bg-card-bg/30 border-white/5"} ${p.id === myId ? "ring-1 ring-primary-accent" : ""}`}
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      {i === 0 && (
                        <Crown className="w-4 h-4 text-[#FACC15] fill-[#FACC15]" />
                      )}
                      <PlayerAvatar
                        seed={p.id}
                        name={p.name}
